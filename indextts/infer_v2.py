...
DEFAULT_HF_CACHE = Path(__file__).resolve().parents[1] / "checkpoints" / "hf_cache_data"
hf_cache_dir = Path(os.environ.get("HF_HUB_CACHE") or DEFAULT_HF_CACHE)
...
class IndexTTS2:
    ...
    def __init__(
        ...
        self.strip_sentence_punctuation = strip_sentence_punctuation
        self.use_vi_cleaner = ViCleaner is not None
        if not self.use_vi_cleaner:
            print(">> ViCleaner not found; using raw text.")

        self.qwen_emo = None
        qwen_path = self.cfg.get("qwen_emo_path")
        if qwen_path:
            qwen_dir = os.path.join(self.model_dir, qwen_path).strip()
            if os.path.isdir(qwen_dir):
                try:
                    self.qwen_emo = QwenEmotion(qwen_dir)
                except Exception as exc:
                    print(f">> Failed to load QwenEmotion from {qwen_dir}: {exc}. Emo text guidance disabled.")
            else:
                print(f">> Qwen emotion directory not found ({qwen_dir}); emo text guidance disabled.")
        ...
        # load campplus_model (optional)
        self.campplus_dim = 192
        ...
        try:
            ...
            self.campplus_model = None
            print(f">> Failed to load CAMPPlus weights ({exc}); style embedding guidance disabled.")
    ...
    def _split_text_into_sentences(self, text: str):
        sentences = []
        endings = "".join(self.SENTENCE_ENDINGS)
        current = []
        for char in text:
            current.append(char)
            if char in self.SENTENCE_ENDINGS:
                sentence = "".join(current)
                if self.strip_sentence_punctuation:
                    sentence = sentence.rstrip(endings)
        ...
        if self.strip_sentence_punctuation:
            remainder = remainder.rstrip(endings)
    ...
    @staticmethod
    def _crossfade_segments(segments, fade_samples: int):
        if not segments:
            raise RuntimeError("No segments to crossfade.")
        output = segments[0].clone()
        for seg in segments[1:]:
            fade = min(fade_samples, output.shape[-1], seg.shape[-1])
            if fade > 0:
                ramp = torch.linspace(0, 1, fade, device=output.device, dtype=output.dtype).unsqueeze(0)
                output[:, -fade:] = output[:, -fade:] * (1 - ramp) + seg[:, :fade] * ramp
                output = torch.cat([output, seg[:, fade:]], dim=-1)
            else:
                output = torch.cat([output, seg], dim=-1)
        return output

    def _synthesize_sentence_to_tensor(
        self,
        text,
        spk_audio_prompt,
        emo_audio_prompt,
        ...
        **generation_kwargs,
        ...
        if not outputs:
            raise RuntimeError("Synthesis produced no data.")
        sampling_rate, wav_np = outputs[0]
        wav_tensor = torch.from_numpy(wav_np.T).to(torch.float32)
        trim = int(sampling_rate * self.SENTENCE_TAIL_TRIM_MS / 1000)
        if trim > 0 and wav_tensor.shape[-1] > trim:
            wav_tensor = wav_tensor[:, :-trim]
        return sampling_rate, wav_tensor

    def _merge_segments_and_save(self, segments, sampling_rate, output_path):
        fade_samples = max(1, int(sampling_rate * self.CROSSFADE_MS / 1000))
        merged = self._crossfade_segments(segments, fade_samples)
        wav_int16 = merged.clamp(-32767, 32767).to(torch.int16)
        if output_path:
            save_audio(output_path, wav_int16.cpu(), sampling_rate)
            return output_path
        wav_data = wav_int16.squeeze(0).unsqueeze(1).cpu().numpy()
    ...
    def infer(self, spk_audio_prompt, text, output_path,
        ...
        for sentence in sentences:
            result = list(self.infer_generator(
                spk_audio_prompt, sentence, None,
                emo_audio_prompt, emo_alpha,
                emo_vector,
                use_emo_text, emo_text, use_random, interval_silence,
                verbose, max_text_tokens_per_segment, stream_return, quick_streaming_tokens=more_segment_before, **generation_kwargs
        ...
        sampling_rate = outputs[0][0]
        merged = torch.cat([torch.from_numpy(wav.T).to(torch.float32) for _, wav in outputs], dim=1)
        wav_int16 = merged.clamp(-32767, 32767).to(torch.int16)
        if output_path:
            save_audio(output_path, wav_int16.cpu(), sampling_rate)
            return output_path
    ...
    def infer_generator(self, spk_audio_prompt, text, output_path,
        ...
        if use_emo_text:
            if self.qwen_emo is None:
                raise RuntimeError("Emotion text guidance requested, but QwenEmotion assets are unavailable.")
        ...
        if output_path:
            ...
            save_audio(output_path, wav.type(torch.int16).cpu(), sampling_rate)
