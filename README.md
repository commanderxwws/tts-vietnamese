...
### 🇻🇳 Vietnamese Fine-Tuning & Inference

1. **Chuẩn bị metadata + audio**  
   - Đặt tất cả câu tiếng Việt vào `metadata.csv` với định dạng `audio_path|text|speaker_id`.  
   - Audio nằm trong một thư mục đã mount sẵn (ví dụ `/mnt/datasets/vivoice/`).

2. **Chạy pipeline tiền xử lý (Steps 1–5)**  
   Sử dụng script tiện ích để chuyển CSV → JSONL, huấn luyện tokenizer, tạo config, cache đặc trưng và sinh manifest prompt/target:
   ```bash
   VOCAB_SIZE=12000 \
   AUDIO_ROOT=/path/to/audio \
   BASE_CHECKPOINT=checkpoints/gpt.pth \
   PYTHON_BIN=python \
   bash tools/run_vi_pipeline.sh
   ```
   Tùy chỉnh `VOCAB_SIZE`, `AUDIO_ROOT`, hoặc `BASE_CHECKPOINT` (nếu checkpoint gốc của bạn nằm ở vị trí khác).

3. **Huấn luyện GPT Việt**  
   Sau khi `processed_vi/gpt_pairs_*.jsonl` được tạo, khởi động huấn luyện:
   ```bash
   uv run python trainers/train_gpt_v2.py \
       --train-manifest processed_vi/gpt_pairs_train.jsonl \
       --val-manifest processed_vi/gpt_pairs_val.jsonl \
       --tokenizer checkpoints/vietnamese_bpe.model \
       --config checkpoints/config_vi.yaml \
       --base-checkpoint checkpoints/gpt.pth \
       --output-dir trained_ckpts_vi \
       --batch-size 32 \
       --grad-accumulation 1 \
       --epochs 10 \
       --learning-rate 1e-5 \
       ...
       --resume auto
   ```
   Điều chỉnh batch size hoặc số epoch theo tài nguyên GPU của bạn. TensorBoard logs nằm trong `trained_ckpts_vi/logs/`.

4. **Suy luận (Inference)**  
   Khi đã chọn được checkpoint tốt nhất, dùng script inference có sẵn:
   ```bash
   uv run python inference_script.py \
       --config checkpoints/config_vi.yaml \
       --gpt-checkpoint trained_ckpts_vi/model_stepXXXX.pth \
       --tokenizer checkpoints/vietnamese_bpe.model \
       --text "Xin chào mọi người, đây là bản demo." \
       --prompt-audio /path/to/prompt.wav \
       --output output.wav
   ```
   Thay `model_stepXXXX.pth` bằng checkpoint mong muốn và cung cấp file prompt phù hợp.

Các bước trên cũng áp dụng cho ngôn ngữ khác: chỉ cần cập nhật metadata, tokenizer, config và manifest tương ứng.
