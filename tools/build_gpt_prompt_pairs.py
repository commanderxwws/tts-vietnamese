...
class Sample:
    ...
    def speaker(self) -> str:
        ...
        if not match:
            # As a last resort, treat the stem itself as the speaker ID so we
            # can continue grouping (likely resulting in singletons that get
            # skipped later).
            return stem or self.record.get("id", "unknown")
