...
def build_unified_voice(cfg, checkpoint: Path, device: torch.device) -> UnifiedVoice:
    ...
    resizable_keys = {
        "text_embedding.weight": gpt.text_embedding.weight,
        "text_head.weight": gpt.text_head.weight,
        "text_head.bias": gpt.text_head.bias,
    }
    for key, param in resizable_keys.items():
        ...
        if weight.shape != param.shape:
            with torch.no_grad():
                if weight.ndim == 1:
                    length = min(weight.shape[0], param.shape[0])
                    param[:length].copy_(weight[:length])
                else:
                    rows = min(weight.shape[0], param.shape[0])
                    cols = min(weight.shape[1], param.shape[1])
                    param[:rows, :cols].copy_(weight[:rows, :cols])
...
def main() -> None:
    ...
    tokenizer = TextTokenizer(str(args.tokenizer), TextNormalizer(preferred_language=args.language))
    tokenizer_vocab = tokenizer.vocab_size
    cfg_limit = int(OmegaConf.select(cfg, "gpt.number_text_tokens", tokenizer_vocab))
    if cfg_limit < tokenizer_vocab:
        cfg.gpt.number_text_tokens = tokenizer_vocab
    ...
    try:
        with open(manifest_path, "r", encoding="utf-8") as source:
            for line in tqdm(source, desc="Preprocessing", unit="sample"):
                ...
                processed += 1
                if assign_to_validation(entry["id"], args.val_ratio):
    ...
    print(f"[Done] processed={processed} skipped={skipped} train={len(train_ids)} val={len(val_ids)}")
