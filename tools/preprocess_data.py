...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Additional directories to search when resolving relative audio paths. "
...
def preprocess_dataset(
    ...
    extra_audio_roots = []
    if getattr(args, "audio_root", None):
        extra_audio_roots = [
            Path(root).expanduser().resolve() for root in args.audio_root if root
        ]
    default_audio_roots = [
        Path(".").resolve(),
    ...
    audio_roots = list(dict.fromkeys(extra_audio_roots + default_audio_roots))
